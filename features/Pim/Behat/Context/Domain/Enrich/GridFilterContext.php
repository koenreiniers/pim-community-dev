...
use Pim\Behat\Decorator\PageDecorator\FilterCapableDecorator;
...
class GridFilterContext extends PimContext
{
    ...
    public function iShowTheFilter($filterName)
    {
        if (false === strpos(strtolower($filterName), 'category')) {
            $filters = $this->getCurrentPage()->getFilters();
            $filters->clickManageFilters();

            $this->getCurrentPage()->getFilterManager()->activateFilter($filterName);
            $filters->assertFilterVisible($filterName);
    ...
    public function iHideTheFilter($filterName)
    {
        if (false === strpos(strtolower($filterName), 'category')) {
            ...
            $this->getCurrentPage()->getFilterManager()->deactivateFilter($filterName);

            $filters->assertFilterNotVisible($filterName);
            $filters->clickManageFilters();
        }
    }
    ...
    public function iShouldSeeTheFilters($not, $filters)
    {
        ...
        $gridFilters = $this->getCurrentPage()->getFilters();
    ...
    public function iShouldSeeTheAvailableFilters($not, $filters)
    {
        $available = !(bool)$not;
        $filters = $this->getMainContext()->listToArray($filters);
        $this->getCurrentPage()->getFilters()->clickManageFilters();
        $filterManager = $this->getCurrentPage()->getFilterManager();
    ...
    public function iFilterBy($filterName, $value)
    {
        ...
        if (preg_match($operatorPattern, $value, $matches)) {
            ...
            $operators = [
                'contains'         => FilterCapableDecorator::FILTER_CONTAINS,
                'does not contain' => FilterCapableDecorator::FILTER_DOES_NOT_CONTAIN,
                'is equal to'      => FilterCapableDecorator::FILTER_IS_EQUAL_TO,
                'starts with'      => FilterCapableDecorator::FILTER_STARTS_WITH,
                'ends with'        => FilterCapableDecorator::FILTER_ENDS_WITH,
                'empty'            => FilterCapableDecorator::FILTER_IS_EMPTY,
                'in list'          => FilterCapableDecorator::FILTER_IN_LIST,
            ];

            $operator = $operators[$operator];
        }

        $this->getCurrentPage()->getFilters()->filterBy($filterName, $value, $operator, $this->getSession()->getDriver());
        $this->wait();
    }
    ...
    protected function filterByDate($filterName, $values, $operator)
    {
        ...
        $filter = $this->getCurrentPage()->getFilters($filterName);
