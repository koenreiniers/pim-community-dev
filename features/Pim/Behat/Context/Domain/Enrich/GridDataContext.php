...
class GridDataContext extends PimContext
{
    ...
    public function theGridShouldContainElement($count)
    {
        ...
        if (0 === $count) {
            assertTrue($this->getCurrentPage()->getCurrentGrid()->isGridEmpty());
            return;
        }
        ...
        $this->spin(function () use ($count) {
            assertEquals(
                $count,
                $actualCount = $this->getCurrentPage()->getCurrentGrid()->getToolbarCount()
            );
            ...
            $count,
            $this->getCurrentPage()->getCurrentGrid()->countRows()
        ));

        $this->spin(function () use ($count) {
            assertEquals(
                $count,
                $actualCount = $this->getCurrentPage()->getCurrentGrid()->countRows()
            );
    ...
    public function iSortByValue($columnName, $order = 'ascending')
    {
        ...
        $loadlingMask = $this->getCurrentPage()->getCurrentGrid()
